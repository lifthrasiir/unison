...
/// An IDC line's `ifexists` is the line's flag and survives a round trip; a
/// component keeps its name whatever it looks like, which is the same rule the
/// gaps are told apart by.
#[test]
fn idc_ifexists_roundtrips() {
    let input = "\
glyph foo 4 2
\u{2FF0} a:2x2 -1 b:2x2 ifexists // split
";
    let doc = parse_document_from_str(input, "test.unf".into()).unwrap();
    let DocumentItem::Glyph { body, .. } = &doc.items[0] else {
        panic!("expected a glyph");
    };
    let compose = &body.compose[0];
    assert!(compose.if_exists);
    assert_eq!(
        compose.part_names().collect::<Vec<_>>(),
        vec!["a:2x2", "b:2x2"]
    );
    assert_eq!(compose.items.len(), 3, "the gap is still an item");

    let mut output = Vec::new();
    serialize_document(&doc, &mut output).unwrap();
    assert_eq!(String::from_utf8(output).unwrap(), input);

    // A line with one token names a glyph called `ifexists`, exactly as the
    // last token of a `map` does.
    let doc =
        parse_document_from_str("glyph foo 4 2\n\u{2FF0} ifexists\n", "t.unf".into()).unwrap();
    let DocumentItem::Glyph { body, .. } = &doc.items[0] else {
        panic!("expected a glyph");
    };
    assert!(!body.compose[0].if_exists);
    assert_eq!(
        body.compose[0].part_names().collect::<Vec<_>>(),
        vec!["ifexists"]
    );
}
