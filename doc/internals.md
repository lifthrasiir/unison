...
| Why a glyph that is one `ref` at an offset carries its component's outline itself, which components keep a glyph id and which are spliced or drawn in place | `render/ttf_builder/absorb.rs` (`absorb_offset_components`, `dissolve_single_use`) |
...
| Why the minimap is a texture and not a mesh, and what it keeps between frames | `editor/minimap.rs` (`MinimapCache`, `build_strip_image`) |
| The gutter's and the minimap's marks of unsaved changes: why the diff pairs lines by id, what "saved" compares against, how a mark spans a line's rows | `editor/change_marks.rs`, `editor/undo.rs` (`UndoStack::revision`), `app/save.rs` (`SaveJob::lines`) |
